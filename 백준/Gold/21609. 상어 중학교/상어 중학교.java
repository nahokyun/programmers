...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			// 블록 탐색 종료
			if (curBlock.size() < 2) {//블록 그룹의 조건(2개이상)을 만족하지 않으면 탈출
				break;
			}
	...
	private static boolean isChange(ArrayList<Point> curBlock, ArrayList<Point> tmp) {
		if (curBlock.size() < tmp.size()) {
			...
			if (cb_rainbow < tmp_rainbow) {// 무지개색 블록이 더 많으면 교체
				return true;
			} else if (cb_rainbow == tmp_rainbow) {
				if (curBlock.get(0).y < tmp.get(0).y) {// 기준블록 y좌표가 더 높으면 교체
					return true;
				} else if (curBlock.get(0).y == tmp.get(0).y) {
					return curBlock.get(0).x < tmp.get(0).x;// 기준블록 x좌표가 더높으면 교체
				}
			}
